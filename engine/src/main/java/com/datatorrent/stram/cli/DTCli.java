...
public class DTCli
{
  ...
  private void checkPlatformCompatible(AppPackage ap)
  {
    ...
    if (!VersionInfo.isCompatible(VersionInfo.getVersion(), apVersion)) {
      throw new CliException("This App Package is compiled with Apache Apex Core API version " + apVersion
          + ", which is incompatible with this Apex Core version " + VersionInfo.getVersion());
    }
  }

  private void launchAppPackage(AppPackage ap, ConfigPackage cp, LaunchCommandLineInfo commandLineInfo,
  ...
  public static class LaunchCommandLineOptions
  {
    ...
    final Option queue = add(OptionBuilder.withArgName("queue name").hasArg()
        .withDescription("Specify the queue to launch the application").create("queue"));
  ...
  public static Options getShowLogicalPlanCommandLineOptions()
  {
    ...
    Option flatten = new Option("r",
        "Flatten the logical plan to display inner modules ( if present ),operators and streams");
